  ...
  const results = useMemo<ResultItem[]>(() => {
    ...
    for (const c of clipboard) {
      if (c.content.toLowerCase().includes(q) || c.preview.toLowerCase().includes(q)) {
        out.push({ kind: "clipboard", id: c.id, title: c.preview, subtitle: `Clipboard - ${c.category}` });
      }
    }
  ...
  return (
    <div>
      <div className="section-header">
        <h2 className="section-title">Universal Search</h2>
      </div>
      <div className="search-bar" style={{ marginBottom: 12 }}>
        <span>{"\u{1F50D}"}</span>
        <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search clipboard, snippets, and transcriptions..." />
      </div>
      {loading ? (
        <div className="empty-state"><div className="spinner" /></div>
      ) : query.trim() === "" ? (
        <div className="empty-state"><p>Type to search across all your data.</p></div>
      ) : results.length === 0 ? (
        ...
        <div className="grid">
          {results.map((r) => (
            <div key={`${r.kind}:${r.id}`} className="card">
              <div style={{ fontSize: 12, color: "var(--text-tertiary)", marginBottom: 4 }}>{r.kind}</div>
